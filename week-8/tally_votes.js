...
// Pseudocode
// 1. Loop through the vote entries
// 2. call function to add property for each new candidate
// 3. if candidate property exists, add 1 to value
// 4. when loop ends, allocate winner to office object
...
for (var office in voteCount) {
  var count = 0;
  for (var candidate in voteCount[office]) {
    if (voteCount[office][candidate] > count) {
      var officer = candidate;
      count = voteCount[office][officer];
    }
  }
  officers[office] = officer;
}
